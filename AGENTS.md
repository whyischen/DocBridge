...
> 优秀工程代码必须遵循「单一实现原则」：任何功能只能有一个实现，其他所有需要该功能的地方都应调用该实现。发现重复代码时，立即重构提取公共模块，严禁复制粘贴。代码冗余是技术债务的主要来源之一，必须持续消除。
>
>优秀代码应遵循 **"卫语句原则"** ：所有异常情况在方法开头用 if 提前返回或抛出异常，核心业务逻辑保持平坦、线性、可读。大量使用 if-else 通常意味着方法职责不单一，需要重构。
...
5. **日志记录**：
   - logger = 给开发者/运维看的技术日志
   - console.print = 给终端用户看的界面反馈
